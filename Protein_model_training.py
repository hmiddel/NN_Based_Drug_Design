...
def protein_train(dims, prot_seq, n, window_size, negative_size):
    """
    ...
    :param prot_seq: input data in the form of an iterable containing protein sequences
    ...
    """
    texts = []
    prot_words = [[] for _ in range(len(list(prot_seq)))]
    get_tmpfile("protein_embedding.model")
    for (j, document) in enumerate(list(prot_seq)):
